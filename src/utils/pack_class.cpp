...
static void check_enough(const size_t remained, const size_t needed) {
    if (remained < needed) {
        throw BytesNotEnoughError((string("there aren't enough bytes to pop (") + to_string(needed) + " bytes needed)").c_str());
    }
}
