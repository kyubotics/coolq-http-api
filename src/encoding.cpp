...
{
    return multibyte_to_widechar(CP_UTF8, utf8_str);
}
...
{
    auto wchar_str = multibyte_to_widechar(CP_UTF8, utf8_str);
    auto gbk_str = widechar_to_multibyte(CP_ACP, wchar_str);
    if (wchar_str)
        free(wchar_str);
    return gbk_str;
}
...
{
    auto wchar_str = multibyte_to_widechar(CP_ACP, gbk_str);
    auto utf8_str = widechar_to_multibyte(CP_UTF8, wchar_str);
    if (wchar_str)
        free(wchar_str);
    return utf8_str;
}
