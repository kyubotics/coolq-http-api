...
namespace cqhttp {
    struct Plugin {
        ...
        virtual void hook_before_action(const std::string &name, json &params, const Next next) { next(); }
        virtual void hook_after_action(const std::string &name, json &result, const Next next) { next(); }
    };
}
