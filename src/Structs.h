...
struct Stranger {
    ...
    json_t *json() const {
        ...
        json_object_set_new(data, "nickname", json_string(encode(nickname).c_str()));
...
struct Group {
    ...
    json_t *json() const {
        ...
        json_object_set_new(data, "group_name", json_string(encode(group_name).c_str()));
        return data;
    }
...
struct GroupMember {
    ...
    json_t *json() const {
        ...
        json_object_set_new(data, "user_id", json_integer(user_id));
        json_object_set_new(data, "nickname", json_string(encode(nickname).c_str()));
        json_object_set_new(data, "card", json_string(encode(card).c_str()));
        json_object_set_new(data, "sex", json_string(sex == 0 ? "male" : sex == 1 ? "female" : "unknown"));
        json_object_set_new(data, "age", json_integer(age));
        json_object_set_new(data, "area", json_string(encode(area).c_str()));
        ...
        json_object_set_new(data, "level", json_string(encode(level).c_str()));
        ...
        json_object_set_new(data, "title", json_string(encode(title).c_str()));
